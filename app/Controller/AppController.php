...
/**
 * Over ride a controllers default redirect action by adding a form field which
 ...
 *
 * @todo Need to move the bottom three to the Theme class along with all of the
 * theme / template stuff in this file.
 */
	...
	protected function _rememberMe() {
		...
		if (!$this->Auth->loggedIn() && $this->Cookie->read('rememberMe')) {
			...
			if ($user && !$this->Auth->login($user['User'])) {
				$this->redirect('/users/users/logout');
 ...
 * This element will call the same page it is sitting on, and add a get variable
 * to the end
 * We want to take that get variable and redirect to the current page with those
 * variables on the end.
 *
 * if the variable contextSorter is set we redirect to
 * http://domain/current_url/contextSorterVar
 *
 * @return null
 */
 ...
 */
	private function _getPaginatorVars($object, $field) {
		...
		if (@$Object->name) {
			...
			$options['fieldValue'] = substr($field, strpos($field, ':') + 1);
			// returns 'incart' from 'status:incart'
			$options['fieldValue'] = $options['fieldValue'] == 'null' ? null : $options['fieldValue'];
			// handle null as a value
			return $options;
		} else {
			return null;
		}
 ...
 */
	private function __handlePaginatorArchivable($object) {
		...
		if (!empty($options['schema']['is_archived'])) {
			$this->redirect(Router::reverse($this->request->params + array(
				'filter' => 'archived:0',
				'url' => null
			)));
		}
 ...
 */
	private function __paginatorFieldName($string, $modelFields = array()) {
		$fieldName = Inflector::underscore(substr($string, 0, strpos($string, ':')));
		// standardizes various name versions
 ...
 * loading abilities.
 *
 * You can create a comma separated (no spaces) list if you only need a system
 * wide component.  If you would like to specify components on a per controller
 * basis, then you use ControllerName[] = Plugin.Component. (ie. Projects[] =
 * Ratings.Ratings).  If you want both per controller, and system wide, then use
 * the key components[] = Plugin.Component for each system wide component to
 * load.  Note: You cannot have a comma separated list, and the named list at the
 * same time.
 */
 ...
 */
	private function _getUses() {
		if (!empty($this->request)) {// this is so that it doesn't load during console
			// activities
 ...
 * Site status
 *
 * @todo	Deprecated, remove references and delete this function OR Upgrade to a
 ...
 */
	private function _configAuth() {
		...
		$this->Auth->loginAction = defined('__APP_LOGIN_ACTION') ? __APP_LOGIN_ACTION : array(
			'plugin' => 'users',
			'controller' => 'users',
			'action' => 'login'
		);
 ...
 *
 * @todo Auth->action() didn't work, but we can get the actual action mapping at
 * some point
 */
	protected function _checkGuestAccess() {
		...
		if ($allowed === 0 || $allowed > 0) {
			...
			$aro = $this->_guestsAro();
			// guests group aro model and foreign_key
			$this->acoPath = $this->_getAcoPath();
 ...
 */
	public function isAuthorized($user) {
		...
		/**
		 * We don't need to check guest access because this function is only called if
		 * we're logged in.
		 * Also there is already a check in another function that fires before this to
		 * check guest access
		 *
		 *
		 // check guest access
		 ...
		 */
		// get paths
		$aro = $this->_userAro($user['id']);
		// user aro model and foreign_key
		$aco = $this->_getAcoPath();
		// get aco
		/**
 ...
 * VERY IMPORTANT : If the aco is a record level type of aco (ie. model and
 * foreign_key lookup) that means that all groups and users who have access
 * rights must be defined.  You cannot have negative values for access
 * permissions, and thats okay, because we deny everything by default.
 *
 ...
 */
	private function _getAcoPath() {
		if (!empty($this->request->params['pass'][0])) {
			...
			$aco = $this->Acl->Aco->find('count', array('conditions' => array(
		...
		if (!empty($aco)) {
			...
			// $aco = 'controllers/Webpages/Webpages/view'; // you could do the full path,
			// but the shorter path is slightly faster. But it does not allow name
			// collisions. (the full path would allow name collisions, and be slightly
			// slower).
 ...
 */
	private function _guestsAro() {
		...
		if (defined('__SYSTEM_GUESTS_USER_ROLE_ID')) {
			$guestsAro = array(
				'model' => 'UserRole',
				'foreign_key' => __SYSTEM_GUESTS_USER_ROLE_ID
			);
		}
		return $guestsAro;
	}
 ...
 *
 * @todo needs comments about why this function is here. Especially because it's
 * not called in this controller.
