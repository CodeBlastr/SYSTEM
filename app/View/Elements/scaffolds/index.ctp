...
/** 
 * Index scaffold
 * Made for easy setup of index / list pages
 * 
 * @todo The <li> part of this should be split into a separate element
 */
...
?>
	...
	foreach ($data as $dat) {
		...
		$name = !empty($displayDescription) ? __('<h3>%s</h3>', strip_tags($name)) : $name;
		...
		$thumbnail = !empty($showGallery) ? $this->Element('thumb', array('model' => $galleryModelName, 'foreignKey' => $galleryForeignKey, 'showDefault' => 'false', 'thumbSize' => $galleryThumbSize), array('plugin' => 'galleries')) : ''; 
		...
		foreach($dat[$modelName] as $keyName => $keyValue) {
			...
			$keyName = str_replace(' ', '', Inflector::humanize(str_replace('_id', '', $keyName))); 
       		if(strpos($_keyName, '_') && !empty($dat[$modelName][str_replace(' ', '', $keyName)]) && is_array($dat[$modelName][str_replace(' ', '', $keyName)])) { 
				...
				$description .= __('<span class="metaDataLi %s"><span class="metaDataLabel  %s"> %s </span><span class="metaDataDetail" name="%s" id="%s">%s</span></span>', $keyName, $keyName, Inflector::humanize(Inflector::underscore($keyName)), $keyName, $id, $keyValue);
			}
		} // end metadata loop
		  
		$description = !empty($displayDescription) ? __('<p>%s</p>', strip_tags($description)) : ''; 
		echo __('<li data-filtertext="%s">%s</li>', $name, $this->Html->link(__('%s %s %s', $thumbnail, $name, $description), $viewUrl, array_merge($viewUrlOptions + array('escape' => false, 'data-transition' => 'flip')))); 
		$s = "";
