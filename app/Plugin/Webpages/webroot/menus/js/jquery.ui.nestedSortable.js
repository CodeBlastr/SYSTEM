	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		toHierarchy: function(options) {
			...
			function _recursiveItems(item) {
        		var id = ($(item).attr(o.attribute || 'data-identifier') || '');
				// var id = ($(item).attr(o.attribute || 'id') || '').match(o.expression || (/(.+)[-=_](.+)/)); // zuha changed
				if (id) {
					var currentItem = {"id" : id};
    				// var currentItem = {"id" : id[2]}; // zuha changed
		...
		toArray: function(options) {
			...
			function _recursiveArray(item, depth, left) {
        		...
        		id = ($(item).attr(o.attribute || 'data-identifier'));
				//id = ($(item).attr(o.attribute || 'id')).match(o.expression || (/(.+)[-=_](.+)/)); // zuha changed
				if (depth === sDepth + 1) {
    			//if (depth === sDepth + 1) { // zuha changed
					pid = o.rootID;
				} else {
					//var parentItem = ($(item).parent(o.listType)
					//						 .parent(o.items)
					//						 .attr(o.attribute || 'id'))
					//						 .match(o.expression || (/(.+)[-=_](.+)/)); // zuha changed
    				var parentItem = ($(item).parent(o.listType)
											 .parent(o.items)
											 .attr(o.attribute || 'data-identifier'));
					pid = parentItem;
    				// pid = parentItem[2]; // zuha changed
				}

				if (id) {
    					ret.push({"item_id": id, "parent_id": pid, "depth": depth, "left": left, "right": right});
						// ret.push({"item_id": id[2], "parent_id": pid, "depth": depth, "left": left, "right": right}); // zuha changed
				}
