  ...
  function logout($options = array()){
    ...
    if(isset($options['redirect']) || $options['custom']){
      if(isset($options['redirect']) && $options['redirect']){
        ...
        $response = "window.location.href = '{$options['redirect']}';";
      } else {
        $response = "window.location.reload();";
      }
      $onclick = "FB.logout(function(response){window.location.href ='/users/users/logout';});";
      //$onclick = "FB.logout(function(response){".$response."});";
      if(isset($options['confirm'])){
        $onclick = 'if(confirm("'.$options['confirm'].'")){'.$onclick.'}';
      }
      return $this->Html->link($options['label'], '/users/users/logout', array('onclick' => $onclick));
    } else {
    	...
    	$init .=  $this->Html->scriptBlock(
      	"
		      window.fbAsyncInit = function() {
				...
				window.location.href = '/facebook/facebook/register';
				});
				FB.Event.subscribe('auth.logout', function() {
					window.location.href = '/users/users/logout';
    			});
		      };
		      (function() {
