...
class ConnectComponent extends Object {
  ...
  function initialize(&$Controller, $settings = array()){
	...
	$this->Controller->Session->write(array('FB' => $this->session));
  ...
  function __syncFacebookUser(){
    ...
    if($Auth->user()){
      ...
      elseif(empty($user) && $this->createUser) {
      	//$this->Controller->redirect(array('plugin' => 'users', 'controller' => 'users', 'action' => 'register_facebook'));
		/*App::import('Lib', 'Facebook.FB');
		$Facebook = new FB();
		$fbUser = $Facebook->api('/me');
        $user[$this->User->alias]['username'] = strtolower($fbUser['first_name']).$this->session['uid'];
        $user[$this->User->alias]['full_name'] = $fbUser['name'];
        $user[$this->User->alias]['first_name'] = $fbUser['first_name'];
        $user[$this->User->alias]['last_name'] = $fbUser['last_name'];
        $user[$this->User->alias]['email'] = $fbUser['email'];
        
        if (defined('__APP_DEFAULT_USER_REGISTRATION_ROLE_ID')) { 
			$user[$this->User->alias]['user_role_id'] = __APP_DEFAULT_USER_REGISTRATION_ROLE_ID; 
		} else {
			$user[$this->User->alias]['user_role_id'] = 3; 
		}
		$user[$this->User->alias]['username'] = $this->session['uid'];
