<?php
/**
 * Acl Extras Shell.  COPIED FROM THE SHELL
 *
 ...
 */
class AclExtrasComponent extends Object {
/**
...
/**
 * Message back to calling controller
 *
 * @var array
 **/
	var $out = array();
 ...
 * @return void
 **/
	function aco_sync($object = array()) {
		$this->_clean = true;
		$this->aco_update($object);
	}
/**
 ...
 * @return void
 **/
	function aco_update($object = array()) {
		$root = $this->_checkNode($this->rootNode, $this->rootNode, null);
		if (!empty($object['controllers'])) :
			$controllers = is_array($object['controllers']) ? $object['controllers'] : array($object['controllers']);
			debug($root);
			debug($controllers);
			$this->_updateControllers($root, $controllers);
		elseif (!empty($object['plugins'])) :
			$plugins = is_array($object['plugins']) ? $object['plugins'] : array($object['plugins']);
 ...
 */
	function _updateControllers($root, $controllers, $plugin = null) {
		...
		if ($this->_clean) {
			if (!$plugin) {
				$controllers = array_merge($controllers, App::objects('plugin', null, false), App::objects('controller', null, false));
			}
			$controllerFlip = array_flip($controllers);
			$this->Aco->id = $root['Aco']['id'];
			$controllerNodes = $this->Aco->children(null, true);
			/* Use this to test what would be deleted.
			foreach ($controllerNodes as $ctrlNode) {
				if (!isset($controllerFlip[$ctrlNode['Aco']['alias']])) {
					echo 'Deleted : '.($ctrlNode['Aco']['id']).'<br />';
				} else {
					echo 'Updated : '.($ctrlNode['Aco']['id']).'<br />';
				}
			}
			break;			*/
			foreach ($controllerNodes as $ctrlNode) {
 ...
 */
	function _checkNode($type = 'controller', $path, $alias, $parentId = null) {
		...
		if (!$node) {
			$alias = !empty($alias) ? $alias : $this->rootNode;
