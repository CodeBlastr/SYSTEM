...
'thumbSize' => 'small', 'thumbLink' => '/users/users/view/'.$user['id']), array('plugin' => 'galleries')); echo $this->Html->link($user['full_name'], array('plugin'=>'users','controller' => 'users' , 'action'=>'view' , $user['id']))?></td>
 ...
 ?>
			...
			if ($status == 'moderator') {
				...
				//				echo $this->Html->link('Join', array('plugin'=>'users',
				// 'controller'=>'user_groups' , 'action'=>'join' ,
				// $user['UsersUserGroup']['id']));
			}
		?>
            </td>
	...
	<table>
	  ...
	  <?php foreach($userGroup['UserGroupWallPost'] as $pgwp) { //debug($pgwp); ?>
		  <tr>
		    <td><?php echo $this->Element('snpsht', array(
				'plugin' => 'users',
				'id' => $pgwp['Creator']['id'],
				'showFirstName' => true,
				'showLastName' => true
 ...
 ?></td>
		    <td>
				<?php
				echo $pgwp['post'] . ' <div class="createdDate"><small>posted ' . $this->Time->niceShort($pgwp['created']) . '</small></div>';
				foreach ($pgwp['Comment'] as $comment) {
					echo $this->Html->tag('div', '<hr />' . $comment['user_id'] . ' said: ' . $comment['body'] . '<br /><small>' . $this->Time->niceShort($comment['created']) . '</small>');
				}
				echo $this->Form->create('Comment', array('url' => '/comments/comments/add')) . $this->Form->hidden('user_id', array('value' => $this->Session->read('Auth.User.id'))) . $this->Form->hidden('model', array('value' => 'UserGroupWallPost')) . $this->Form->hidden('foreign_key', array('value' => $pgwp['id'])) . $this->Form->input('body', array(
					'label' => false,
	...
	<?php #echo $this->Html->link('Join This Group' ,
		# array('plugin'=>'users','controller'=>'users_user_groups' , 'action'=>'add',
		# $userGroup['UserGroup']['id'] , $userId ));
	?>