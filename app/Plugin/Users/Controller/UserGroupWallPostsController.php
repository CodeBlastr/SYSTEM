...
class UserGroupWallPostsController extends UsersAppController {
	...
	function add($group_id) {
		if (!empty($this->data)) {
			$this->data["UserGroupWallPost"]["creator_id"] = $this->Auth->user('id');
			$this->data["UserGroupWallPost"]["user_group_id"] = $group_id;
			$this->UserGroupWallPost->create();
			if ($this->UserGroupWallPost->save($this->data)) {
	...
	function edit($id = null) {
		...
		if (empty($this->data)) {
			$this->data = $this->UserGroupWallPost->read(null, $id);
		}
