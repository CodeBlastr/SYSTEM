...
class UsersUserGroupsController extends UsersAppController {
	 ...
	 */
	function add() {
		if ($groupId = $this->data['UsersUserGroup']['user_group_id'] && $userId = $this->data['UsersUserGroup']['user_id']) : 
			...
			if($cnt == 0){
				...
				if ($this->UsersUserGroup->save($this->data)) {
	...
	function edit($id = null) {
		...
		if (!empty($this->data)) {
			if ($this->UsersUserGroup->save($this->data)) {
				...
				$this->redirect(array('plugin' => 'users', 'controller'=>'user_groups' , 'action'=>'view', $this->data['UsersUserGroup']['user_group_id']));
			} else {
				$this->Session->setFlash('Update failed.');
			}
		}
		if (empty($this->data)) {
			$this->data = $this->UsersUserGroup->read(null, $id);
		}
		$users = $this->UsersUserGroup->User->find('list');
