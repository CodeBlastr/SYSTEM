    ...
    CKEDITOR.plugins.add('oembed', {
        ...
        init: function (editor) {
            ...
            editor.widgets.add('oembed', {
                ...
                template: '<div class="' + (editor.config.oembed_WrapperClass != null ? editor.config.oembed_WrapperClass : "embeddedContent") + '">' +
                    '</div>',
                    ...
                    contents: [
                        ...
                        },
                        {
                            ...
                            elements: [
                                {
                                    type: "html",
                                    html:"<span style='color:red'>mp3,mp4,ogg format only</span>"
                                ...
                                },
                                {
                                    ...
                                    onChange:function(obj){
                                        var allowedFormat = ['mp3','mp4','ogg'];
                                        var ext = obj.data.value.split('.').pop().toLowerCase();

                                        if(allowedFormat.join(',').indexOf(ext) < 0){
                                            alert('Only '+allowedFormat.join(',')+ 'allowed');
                                            this.hasError = true;
                                        }else{
                                            this.hasError = false;
                                        }
                                ...
                                },
                                {
                                    ...
                                    onClick:function(){
                                        var dialog = CKEDITOR.dialog.getCurrent(),
                                            error = dialog.getContentElement('Upload','upload').hasError;
                                        if(error){
                                            alert('file format not allowed, please choose another file');
                                            return false;
                                        }
