...
$(function() {
	...
	$(document.body).on('keyup', '.changer', function() {
		var duplicate = $(this).closest('.duplicatable').clone();
		duplicate.find('input').val('');
		duplicate.find('input').attr('name', function(i, val) {
			value = val.replace(/(.*)\[([0-9]+)\]/, function(f, p1, p2) {
				return p1 + '[' + (parseInt(p2) + 1) + ']';
	   		});
	   		return value;
		});
		$(this).removeClass('changer').closest('.duplicatable').removeClass('duplicatable').after(duplicate);
	});
});
