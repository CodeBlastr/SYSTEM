...
function clone (selection) {
	var duplicate = $(selection).closest('.duplicatable').clone();
	var selectors = 'input, select';
	duplicate.find(selectors).val('');
	duplicate.find(selectors).attr('name', function(i, val) {
		var value = val;
		if (val) {
			value = val.replace(/(.*)\[([0-9]+)\]/, function(f, p1, p2) {
				return p1 + '[' + (parseInt(p2) + 1) + ']';
	   		});
	   	}
   		return value;
	});
	duplicate.find(selectors).attr('id', function(i, val) {
		var value = val;
		if (val) {
			value = val.replace(/(.*)([0-9]+)/, function(f, p1, p2) {
				return p1 + (parseInt(p2) + 1);
	...
	$(selection).removeClass('changer').closest('.duplicatable').removeClass('duplicatable').after(duplicate);
}
 ...
 *  a - Represents an alpha character (eg. A-Z,a-z) ex. <input data-mask="aaa@aaaa.aaa">
 *  9 - Represents a numeric character (eg. 0-9) ex. <input data-mask="(999) 999-9999">
 *  - Represents an alphanumeric character (eg. A-Z,a-z,0-9) ex. <input data-mask="--/--">
 *	Version: 1.3.1
 **/
...
$(function() {
    $('form.form-validate, form.form-split').validate();
	...
	// hash tag support for split forms  (#stepX)
	if (window.location.hash.length > 0) {
		var hash = window.location.hash;
    	$('form.form-split .step').hide();
    	$('form.form-split .step.' + hash.replace('#', '')).show();
		activateNav(hash.replace('#step', ''));
	}
	
    // handle form-split clicks
    $('.form-split').on('click', 'a.next', function() {
		that = $(this);
		if (that.closest('form').valid()) {
    		var step = that.data('next-step');
			$('html, body').animate({scrollTop: that.closest('form').offset().top}, 500);
	    	$('form.form-split .step').hide();
	    	$('form.form-split .step.step' + $(this).attr('data-next-step')).show();
	    	activateNav(step);
	   } else {
	   		e.preventDefault();
	   }
    });
    $('.form-split').on('click', 'a.prev', function() {
		that = $(this);
		if (that.closest('form').valid()) {
	    	var step = that.data('prev-step');
			$('html, body').animate({scrollTop: that.closest('form').offset().top}, 500);
	    	$('form.form-split .step').hide();
	    	$('form.form-split .step.step' + $(this).data('prev-step')).show();
	    	activateNav(step);
	   } else {
	   		e.preventDefault();
	   }
    ...
    function activateNav(step) {
		// update navigation element from the prev and next buttons
    	$('.form-split-navigation ul').children('li').removeClass('active');
    	$('.form-split-navigation a[href="#step' + step + '"]').closest('li').addClass('active');
    }
})
